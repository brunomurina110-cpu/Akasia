...
(() => {
  ...
  const { DEFAULT_CONFIG, NECKLACES_DATA, LOOKBOOK_GALLERY } = window;
...
class AkasiaApp {
  ...
  generateWhatsAppLink(necklace = null) {
    ...
    if (necklace) {
      text = `¡Hola Akasia! ✨ Me interesa el collar exclusivo *${necklace.name}* (${DEFAULT_CONFIG.currency} ${necklace.price}) que vi en su catálogo web (Largo: ${necklace.length}, Diámetro: ${necklace.diameter}). ¿Sigue disponible para compra?`;
    } else {
      text = `¡Hola Akasia! ✨ Vi su catálogo online de collares únicos y me gustaría hacer una consulta personalizada.`;
    }
  ...
  getFilteredNecklaces() {
    return this.necklaces.filter(item => {
      // Filtro de estado
  ...
  openProductModal(productId) {
    const product = this.necklaces.find(p => p.id === productId);
    if (!product) return;
